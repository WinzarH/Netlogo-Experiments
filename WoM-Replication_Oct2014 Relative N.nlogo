...
to setup
  ...
  set size_of_weak_contacts 2 ^ ( nStrong2 + nWeakRelativeToStrong )
  set strong-ties-effect 2 ^ StrongEffect
  set weak-ties-effect 2 ^ ( StrongEffect + WeakEffectRelativeToStrong )
...
to go
  ...
  if not any? turtles with [ buy = 0 ] [ finish stop ] 
  ShowLinks
  set sales sum [ buy ] of turtles
  ;; ask turtle 1 [ show count acquaintances ] 
  ;; ask turtle 20 [ show count acquaintances ]
  record-data
  do-plots
  set clock (clock + 1)  
end
...
;;; unless you're particularly interested, it's safe to ignore the rest of the code.
;;;
;;;
;;; This section of code adapted from the GROUPING TURTLES EXAMPLE from the MODELS LIBRARY
;;; *** NetLogo 4.0beta5 Code Example Copyright Notice ***
...
to gather-turtles
  ...
  [
    ...
    if my-group != -1
      [ face get-home ]
    ;; wiggle a little and always move forward, to make sure turtles don't all pile up
...
to-report get-home ;; turtle procedure
  ...
  ;; compute the x coordinate
    (round ((world-width / side) * (my-group mod side) + min-pxcor + int (world-width / (side * 2))))
  ;; compute the y coordinate
    (round ((world-height / side) * int (my-group / side) + min-pycor + int (world-height / (side * 2))))
end
...
to UpdateAcquaintances
  ;; set acquaintances []
  set acquaintances  n-of size_of_weak_contacts turtles with [ my-group != myself ]
end 
  
to likelihood
  set strong-effect sum [ buy ] of friends 
  set weak-effect sum [ buy ] of acquaintances
  ...
  ifelse influence >= random-float 1
    [ set buy 1   set color orange   set shape "person"]
    [ set buy 0 ]
end 

to ShowLinks
  ifelse LinkDisplay = True
  [
    clear-links
    ask one-of turtles with [ buy = 0 ]
    [
      ;; create-links-with other friends
      create-links-with other acquaintances
    ] 
  ]
  [
    clear-links
...
__Word-of-Mouth-Replication__ is a replication of a study reported in:
...
Key Findings from Goldenberg were:
...
We found with this replication that, while Weak Ties are important, they are not nearly as important as found in the original Goldenberg paper. 
...
__Market Size__ adjusts the number of people in the market.  A very large number makes the model VERY slow with no improvement in consistency of output.  I suggest leaving it at 2^10, i.e. 1024.

__Size of Personal Network__ is the number of people in constant, highly-regarded, contact.  Goldenberg et al used a range from 5 to 29. This model uses a range using base-2, i.e. the number doubles at each increment, starting at 2 and rising to 32. A larger number implies faster communication within the clique.

__Size of Weak Contacts__ is the number of people that each person speaks with in each period from outside the clique.  Goldenberg et al used a range from 5 to 29. This model uses a range using base-2, i.e. the number doubles at each increment, starting at 2 and rising to 32. A larger number implies faster communication between cliques.

__Weak Ties Effect__ is a parameter affecting the influence of the number of weak contacts on awareness likelihood. By definition, Weak Ties Effect is always less than or equal to Strong Ties Effect. In this version we have set Weak Ties Effect at some fraction of Strong Ties Effect, halving at each increment, ranging from 1/1 down to 2^-8 i.e. 1/256.

__Strong Ties Effect__ is a parameer affecting the influence of the number of strong contacts on awareness likelihood. This model uses a range using base-2, i.e. the number halves at each increment, starting at 2^-4 (1/16) and continuing to 2^-8 i.e. 1/256 of the Strong Ties Effect number.

__Advertising Effect__ is a parameter representing the level of non-Word-of-Mouth information received by all citizens in each time period.
...
Wilensky, U. (2004). Grouping Turtles Example http://ccl.northwestern.edu/netlogo/models/GroupingTurtlesExample. Center for Connected Learning and Computer-Based Modeling, Northwestern University, Evanston, IL.
...
@#$#@#$#@
<experiments>
  <experiment name="Experiment 5 relative N 30Sept14" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="nWeakRelativeToStrong">
      <value value="-2"/>
      <value value="-1"/>
      <value value="0"/>
      <value value="1"/>
    ...
    <enumeratedValueSet variable="WeakEffectRelativeToStrong">
      <value value="-4"/>
      <value value="-3"/>
      <value value="-2"/>
      <value value="-1"/>
  ...
  <experiment name="Experiment 3 relative strength small N f" repetitions="25" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>percent16</metric>
    <metric>percent50</metric>
  ...
  <experiment name="Experiment 5 relative N 3Oct14" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>percent16</metric>
    <metric>percent50</metric>
